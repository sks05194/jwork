...
public class BoardDAODBCP {
	...
	private final String BOARD_LIST_TITLE = "SELECT * FROM board WHERE title LIKE CONCAT(CONCAT('%',?), '%') ORDER BY seq DESC";
	private final String BOARD_LIST_CONTENT = "SELECT * FROM board WHERE content LIKE CONCAT(CONCAT('%',?), '%') ORDER BY seq DESC";
