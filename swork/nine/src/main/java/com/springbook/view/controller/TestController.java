...
public class TestController {
	...
	반면에 xml이나 json 기반의 메세지를 사용하는 요청의 경우에 이 방법이 메우 유용하다.
	...
	**jackson-databind 라이브러리가 있는 경우 @ResponseBody를 사용하여
	데이터를 전송하면 데이터의 타입을 확인하여 자동으로 JsonArray이나 jsonObject 자료형으로 변환하여 반환해준다.
...
//		String formatedNow = now.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
	...
	// 커맨드 객체가 자동으로 setter 주입이 가능한 데이터는 form-data(파라미터)만 가능하다.
	// json 형태의 데이터를 setter 주입 불가.
