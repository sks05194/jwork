...
public class Client {
	...
	public static void main(String[] args) {
		...
		try {
			...
			socket = new Socket(ia, 7777); // server의 IP와 port 번호 입력

			/*
			 * DataInputStream은 입력 스트림을 받는 매개변수이며, socket.getInputStream()함수를 통해 소켓에서 전달되는
			 * 데이터 스트림을 읽어옵니다. BufferedReader는 Scanner와 비슷한 개념입니다. Scanner보다 빠르다는 장점이 있지만
			 * String형으로 버퍼에 저장하기 때문에 따로 데이터를 가공해서 사용해야하는 경우가 많습니다.
			 */
			in = new DataInputStream(socket.getInputStream()); // 서버에서 들어온 데이터를 전송받는 객체
			in2 = new BufferedReader(new InputStreamReader(System.in)); // 클라이언트에서 닉네임을 입력할때 사용하는 객체
			out = new DataOutputStream(socket.getOutputStream()); // 서버로 전송할 데이터를 담아두는 객체, 전송객체
		...
		try {
			/*
			 * 끊어지기 전까지는 계속 채팅을 입력받기 때문에 무한루프(while)안에서 동작합니다. 만일 끊고 싶다면 break에 대한 조건문을 걸면
			 * 됩니다.
			 */
			while (true) {
				String str2 = in.readUTF();
				// 클라이언트의 Send 클래스의 run 메소드 안에 있는 out.writeUTF(msg); 코드로 인해 전송된 데이터를 받아오는 곳
