...
public class LoginCheckFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		 ...
		 * 
		 * 2. getSession(false)
		 * HttpSession이 존재하면 현재 HttpSession을 반환하고 존재하지 않으면 객체 생성하지 않고 그냥 null을 반환함.
		 * */
		...
		if (login) {
			...
			//RequestDispatcher: 현재 request에 담고 있는 정보를 저장하고 있다가 그 다음 페이지, 다음 페이지에서도 해당 정보를 볼 수 있게 저장하는 기능을 한다.
