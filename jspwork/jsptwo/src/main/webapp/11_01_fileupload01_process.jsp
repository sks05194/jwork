...
// DefaultFileRenamePolicy() 메서드는 파일 이름이 중복될 경우 파일명 뒤에 1과 같은 숫자를 붙여 파일 이름을 rename 해주는 역할을 한다. 
// MultipartRequest는 객체를 생성하자마자 파일을 바로 업로드한다.
// 파일을 저정할 경로는 직접 써주기 보다는 tomcat의 내장객체인 application 객체의 getRealPath() 메서드를 사용하는 것이 좋다.
...
/*
전송된 파일 타입의 파라미터 이름들을 Enumeration 타입으로 반환한다.
Enumeration은 객체타입을 저장하는 것으로서 형변환이 없이 객체 타입을 자유롭게 저장하고 꺼내기 위해서 사용한다.
MultipartRequest를 사용하게 되면 tomcat의 request 객체의 getParameter 메서드를 이용하여 값을 전달받지 못한다.
