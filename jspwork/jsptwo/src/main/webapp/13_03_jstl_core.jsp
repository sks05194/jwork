...
<body>
    ...
    <%String url = "https://2ham-s.tistory.com/274";%>
    c:out을 사용하는 이유 
    <a href="<%=url%>"><%=url%></a><br>
    ...
    <h2>c:set 태그 - 변수의 값을 설정(setAttribute와 동일한 역할을 한다.)</h2>
    <c:set var="num" value="50" />
	<%-- <c:set var="num">50</c:set> --%>
	<%-- <c:set var="num" value="50" scope="page" /> --%>
    num 값은: ${num}<br><hr>

    <h2>c:out 태그 - 변수의 값을 출력</h2>
    
    <c:out value="${num}" default="0" />,
    <c:out value="작성자" default="Admin" />,
    <c:out value="hello world!!!!!!!" /><br><hr>

    <h2>c:forEach 태그 - 반복문 수행</h2>
    범위 안에서 반복문을 수행한다. for와 동일한 역할을 한다.<br>
    begin: 초기값, end: 마지막 값, step: 증감값<br><br>
    ...
    <h2>c:choose / c:when 태그 - 조건문 수행</h2>
