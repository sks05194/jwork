...
<body>
	...
	// 요청 객체에서 요청 정보 가져옴. HttpServletRequest 객체를 사용해야만 한다.
		...
		for (Part part : parts) {
			...
			if (part.getHeader("Content-Disposition").contains("filename=")) {
				...
				if (part.getSize() > 0) {
					...
					// 실제 파일이 저장되는 시점. // 새 파일 이름을 설정하지 않으면 기존 파일로 덮어쓰기 됨.
					part.write(path + File.separator + fileName);
					part.delete();
					// 임시 저장된 파일 데이터를 제거함.
					// HTTP 요청이 처리되고 나면 자동으로 제거되지만 그 전에 메모리나 디스크 자원을 아끼기 위해 수동으로 제거해준다.
				}
			} else { // 그 외 전달된 파라미터들
				String formValue = request.getParameter(part.getName());
				writer.println("name: " + part.getName() + ", value: " + formValue + "<br><br>");
			}
		}
